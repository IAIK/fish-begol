...
#pragma omp parallel for
  for (unsigned int i = 0; i < BG_NUM_ROUNDS; ++i) {
    c_mpc_y[i] = mpc_lowmc_call(lowmc, &lowmc_key_s[i], p, views_y[i], rvec_y[i]);
...
#pragma omp parallel for reduction(| : view_verify_status)
  for (unsigned int i = 0; i < BG_NUM_ROUNDS; ++i) {
    ...
    if (mpc_lowmc_verify(lowmc, p, proof_y->views[i], rv_y, ch[i]) ||
        mpc_lowmc_verify(lowmc, beta, proof_c->views[i], rv_c, ch[i])) {
...
#pragma omp parallel for reduction(| : output_share_status)
  for (unsigned int i = 0; i < BG_NUM_ROUNDS; ++i) {
    ...
    if (mzd_equal(ys_y[i][a], proof_y->views[i][last_view_index].s[0]) ||
        mzd_equal(ys_y[i][b], proof_y->views[i][last_view_index].s[1]) ||
        mzd_equal(ys_c[i][a], proof_c->views[i][last_view_index].s[0]) ||
        mzd_equal(ys_c[i][b], proof_c->views[i][last_view_index].s[1])) {
